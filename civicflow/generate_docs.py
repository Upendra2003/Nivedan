"""
CivicFlow — Technical Documentation Generator
Run: uv run --with python-docx python generate_docs.py
Output: CivicFlow_Technical_Documentation.docx
"""
...
body(
    'CivicFlow is a multilingual AI assistant that guides Indian citizens through the process '
...
body(
    'CivicFlow is composed of four independently runnable processes that communicate '
...
two_col_table([
    ('civicflow',        'users, complaints, notifications — used by Backend API'),
    ('civicflow_portal', 'portal_submissions — used by Mock Portal'),
...
body(
    'Located at civicflow/backend/. Managed exclusively with uv (never pip). '
...
body(
    'The heart of CivicFlow. Implements a goal-oriented state machine that drives '
...
client = MongoClient(os.getenv("MONGO_URI", "mongodb://localhost:27017/civicflow"))
...
body(
    'Located at civicflow/mobile/. Expo SDK 54 with expo-router file-based routing. '
...
two_col_table([
    ...
    ('agent',                   'Left-aligned bubble with "CivicFlow AI" label + dot.'),
...
body(
    'Located at civicflow/web/. React 18 + react-router-dom + Tailwind CSS (CDN) + Recharts. '
...
body(
    'Located at civicflow/mock_portal/portal_app.py. Simulates a government e-filing portal. '
    'Separate Flask process on port 5001. Has its own MongoDB database (civicflow_portal). '
...
h2('10.1 users collection (civicflow)')
code_block('''\
{
  _id:                ObjectId,
  name:               String,
...
h2('10.2 complaints collection (civicflow)')
code_block('''\
{
  _id:               ObjectId,
  user_id:           ObjectId,
...
h2('10.3 notifications collection (civicflow)')
code_block('''\
{
  _id:          ObjectId,
  user_id:      ObjectId,
...
h2('10.4 portal_submissions collection (civicflow_portal)')
code_block('''\
{
  _id:            ObjectId,
  complaint_id:   String,       // CivicFlow complaint _id
  user_id:        String,
...
# ── Final save ─────────────────────────────────────────────────────────────────
output_path = 'CivicFlow_Technical_Documentation.docx'
