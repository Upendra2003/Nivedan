...
```bash
cd Nivedan/backend    && uv run python app.py        # port 5000
cd Nivedan/mock_portal && uv run python portal_app.py # port 5001
cd Nivedan/web        && npm run dev                  # port 5173
cd Nivedan/mobile     && npx expo start --clear       # scan QR / press a/i/w
```
...
### Mock Portal (`Nivedan/mock_portal/`)

```
portal_app.py             Flask, port 5001, Nivedan_portal DB
...
| DB | Collections |
|----|-------------|
| `Nivedan` | `users`, `complaints`, `notifications` |
| `Nivedan_portal` | `portal_submissions` |

**Wipe for clean test:**
```js
use Nivedan; db.complaints.deleteMany({}); db.notifications.deleteMany({})
use Nivedan_portal; db.portal_submissions.deleteMany({})
```
...
#### Branding
- App renamed **Nivedan** across all surfaces (was Nivedan)
